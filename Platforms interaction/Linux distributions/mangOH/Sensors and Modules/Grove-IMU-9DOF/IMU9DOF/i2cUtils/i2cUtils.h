			       ...
			       __u8 len)
{
	...
	if (i2c_fd < 0) {
		...
		LE_INFO("i2cSendBytes: failed to open %s", i2c_bus);
		return -1;
	}
	if (ioctl(i2c_fd, I2C_SLAVE_FORCE, address) < 0) {
...
)
{
	...
	if (i2c_fd < 0) {
		printf("i2cReceiveBytes: failed to open %s", i2c_bus);
		LE_INFO("i2cReceiveBytes: failed to open %s", i2c_bus);
		return -1;
	}
	if (ioctl(i2c_fd, I2C_SLAVE_FORCE, address) < 0) {
		printf("i2cReceiveBytes: could not set address to 0x%02x: %s\n",
		       address,
		       strerror(errno));
		LE_INFO("i2cReceiveBytes: could not set address to 0x%02x: %s\n",
		       address,
		       strerror(errno));
		close(i2c_fd);
		return -1;
